...
const BlogCard = ({ blog, isCenter, rtl }) => {
  return (
    <div className={`blog-card${isCenter ? ' card-center':''} style-6`}>
      {
        rtl ?
        <Link to="rtl-page-single-post" className="img img-cover d-block">
          <img src={blog.cover} alt="" />
        </Link>
        :
        <Link to="/page-single-post-5" className="img img-cover d-block">
          <img src={blog.cover} alt="" />
        </Link>
      }
      <div className="info">
        ...
        <h4 className="blog-title">
          {
            rtl ?
            <Link to="rtl-page-single-post">{ blog.title }</Link>
            :
            <Link to="/page-single-post-5">{ blog.title }</Link>
          }
        </h4>
        <div className="text">
          { blog.short_desc } [...]
